...
const handleTheatreApproval = async (e, theatre_id) => {
  e.preventDefault(); 
  

  try {
    const response = await axiosInstance.patch(
      `/cadmin/admin/theatre-approve/${theatre_id}/`
    );

    if (response.status === 200) {
      toast.success(`Theatre '${response.data.message}' has been approved.`);
      fetchTheatreRequests()
    } else {
      toast.error("Failed to approve theatre. Please try again.");
    }
  } catch (error) {
...
const handleTheatreDisapproval = async (e, theatre_id) => {
  e.preventDefault(); 
  try {
    const response = await axiosInstance.patch(
      `/cadmin/admin/theatre-disapprove/${theatre_id}/`
    );

    if (response.status === 200) {
      
      toast.success(`Theatre '${response.data.message}' has been disapproved.`);
      fetchTheatreRequests()
    } else {
      toast.error("Failed to disapprove theatre. Please try again.");
    }
  } catch (error) {
    console.error("Error disapproving theatre:", error);
    if (error.response && error.response.data && error.response.data.error) {
      toast.error(`Error: ${error.response.data.error}`);
    } else {
      toast.error("Something went wrong. Please try again later.");
              ...
              <table className="bg-[#2d2e3e] min-w-full border-separate border-spacing-y-2 border-spacing-x-2">
                <thead className="hidden border-b lg:table-header-group">
                  <tr>
                    <td
                      width="50%"
                      className="whitespace-normal py-4 text-sm font-medium text-gray-400 sm:px-6"
                    >
                      Name
                    </td>
                    <td className="whitespace-normal py-4 text-sm font-medium text-gray-400 sm:px-6">
                      Owner
                    </td>
                    <td className="whitespace-normal py-4 text-sm font-medium text-gray-400 sm:px-6">
                      State
                  ...
                  {requests.map((theatre, index) => (
                    <tr className="" key={theatre.id}>
                      ...
                      <td className="whitespace-no-wrap hidden py-4 text-sm font-normal text-gray-500 sm:px-6 lg:table-cell">
                        <button
                          onClick={(e) =>
                            handleTheatreDisapproval(e, theatre.id)
                          }
                        ...
                        <button
                          onClick={(e) => handleTheatreApproval(e, theatre.id)}
                  ...
                  <form className="max-w-sm mx-auto">
                    ...
                    <div className="mb-5">
                      ...
                      <input
                        ...
                        id="details"
                        className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                        value={`${theatre.state} , ${theatre.district} , ${theatre.city}`}
                      />
                    </div>
                    <div className="mb-5">
                      <label
                    ...
                    <div className="mb-5">
                      ...
                      <a
                        href={theatre.google_maps_link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="shadow-sm bg-gray-50 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-[8rem] p-2.5 dark:bg-blue-700 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light hover:border-blue-500"
                      >
