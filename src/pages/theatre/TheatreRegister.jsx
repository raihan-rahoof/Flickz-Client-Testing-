  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status == 201) {
        ...
        toast.success(
          "A verification code has sent to your mail please enter it Here"
        );
      }
    } catch (error) {
      setLoading(false);
    ...
    <>
      <div class="flex-col dark md:flex justify-center items-center h-auto">
        <form
          ...
          class="relative top-[3rem] border border-gray-700 space-y-3 max-w-screen-md mx-auto rounded-md bg-gray-800 p-6 shadow-xl lg:p-10"
        >
          <h1 class="mb-6 text-xl font-semibold lg:text-2xl text-white">
          ...
          <div class="grid gap-3 md:grid-cols-2">
            <div>
              ...
              <input
                ...
                class="mt-2 h-12 w-full rounded-md bg-gray-700 px-3 text-white"
              />
            </div>
            <div>
          ...
          <div>
            {isLoading ? (
              <button
                disabled
                type="submit"
                class="mt-5 w-full rounded-md bg-indigo-500 p-2 text-center font-semibold text-white"
              >
                <div
                  class="animate-spin inline-block size-7 border-[3px] border-current border-t-transparent text-white rounded-full "
                  role="status"
                  aria-label="loading"
                >
